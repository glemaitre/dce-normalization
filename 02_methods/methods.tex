...
In this work, we proposed a method to normalized \ac{dce}-\ac{mri} prostate data.
However, it can be note that this method can be used for any \ac{dce}-\ac{mri} sequences.
The aim of the method is to reduce the intra-patient variation that can occur during the acquisition.
These variations can be highlighted by observing the evolution of the intensity \ac{pdf} of the \ac{dce}-\ac{mri} over time, as shown in Fig.\,\ref{fig:heatmap}.
These variations are:
(i) an offset of the peak value before pre-contrast,
(ii) a time offset depending of the contrast injection, and
(iii) a scale factor related to the enhancement.
Therefore, our normalization method should attenuate all these variations

\subsubsection{Baseline alignment of each \ac{dce}-\ac{mri} serie}

Short-path using graph minimization.

\subsubsection{Estimate the dispersion of the voxel intensities}

MSE.

\subsubsection{Parametric curve alignment}

Model using 1 translation and one scale.
